...
- 非同期ジョブ実行 (バッチ処理)
  ...
  - Lambda と違って実行時間に制約がない
    - (メモ) Azure Functions だと App Service プランにすると実行時間については
...
- クラウドへのマイグレーション
  - レガシーシステムでもコンテナにさえしてしまえば容易にクラウドに持っていける
    - (メモ) 各クラウド プロバイダーが提供しているマイグレーション サービスで
...
- AWS Fargate と Amazon ECS の共存が可能
  - (メモ) クラスターAは Amazon ECS、クラスターB は AWS Fargate というだけで、
...
(メモ) [AWS Fargate][aws-fargate] のページを見ると下記のように記載あり
...
- (メモ) 多分、同じレジストリを参照して別クラスターにデプロイするだけ
